...
function App() {
  ...
  const command20 =
    "Summarize the given text into 20-30 words depending on length of text. ";
  const command40 =
    "Summarize the given text into 40-60 words depending on length of text. ";
  const command70 =
    "Summarize the given text into 70-100 words depending on length of text. ";
    ...
    <div className=" text-white bg w-[660px] h-[400px] font-mono ">
      <div className="bg1 h-full overflow-y-auto w-full text-center text-l text-black">
        ...
        <textarea
          ...
          cols="90"
          rows="10"
          className="my-3 p-3 text-justify border-2 border-black rounded-sm text-xs"
        ></textarea>{" "}
        <br />
        {/* <input type="text" className='px-1 h-16 w-72 border-2 border-black rounded-sm ' onChange={ (e) => setQuestion(e.target.value) } /> <br /> */}
        
        <button
          className="h-10 w-44 bg-green-400 border-2 border-black rounded-md mx-2 hover:bg-cyan-300 hover:scale-105"
          onClick={generate20Answer}
        >
          Short (20-30 Words)
        </button>
        <button
          className="h-10 w-48 bg-green-400 border-2 border-black rounded-md mx-2 hover:bg-cyan-300 hover:scale-105"
        ...
        {copied ? (
          ...
          >
            <button className="h-8 w-48 mb-3 border-2 border-black rounded-md bg-fuchsia-200 hover:bg-fuchsia-600">
              Copy to clipboard
            </button>
          </CopyToClipboard>
        ) : null}
